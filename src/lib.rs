use crate::notation::r#trait::{Notation};
...
pub fn handle(window: &AppWindow) {
    ...
    window.global::<elements>().on_touch(move |char| {
        ...
        let base_expression = &mut pika.borrow_mut();
        ...
        let _string: &str = base_expression.as_ref();
        app.global::<elements>()
            .set_text(SharedString::from(_string));
        ...
        app.global::<elements>()
            .set_paren(SharedString::from(dbg!(&*paren_str)));
    ...
    window.global::<elements>().on_copy(move |string| {
        crate::copy_to_clipboard(string);
    });
}
